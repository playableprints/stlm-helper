...
const FilesToFolders = () => {
  ...
  useEffect(() => {
    if (selected.length > 0) {
      Prepare(selected, matcher === "" ? ".*" : matcher, replace === "" ? "$0" : replace).then(
        (h) => {
    ...
    <>
      ...
      <Panel>
        ...
        <Columns value={"1fr auto 1fr"}>
          ...
          <Label
            text="Folder Names"
            help={"Name the folders. Regex Matches can be accessed here with '$n'."}
          >
            <Input
              value={replace}
              onChange={(e) => {
          ...
          onClick={() => {
            ...
            ExplodeList(
              ...
              if (success) {
                ...
                notifications.error(
                  <>
                    There was a problem creating your folders, check the <Nav to="/">Logs</Nav>
                  </>,
